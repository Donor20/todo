...
function Sidebar() {
  ...
  return (
    <aside className="lg:w-32 flex flex-col lg:mb-6">
      <Card variant="secondary" className="flex-1 flex min-h-0 max-h-150 pt-0 pb-0">
        <nav className="h-full flex lg:flex-col lg:gap-2 overflow-x-auto lg:my-2 lg:justify-between">
          ...
          {theme === 'light' ? (
            <SidebarButton onClick={() => setTheme(toggleTheme())} icon="moon" label="Dark theme" />
          ) : (
            <SidebarButton onClick={() => setTheme(toggleTheme())} icon="sun" label="Light theme" />
          )}
        </nav>
      </Card>
    </aside>
